...
class FlowMatchingScheduler(nn.Module):
    ...
    def sample_euler(
        ...
        Args:
            null_cond: the null conditioning for CFG. Must be a learned
                       nn.Parameter (from DiTTrainer.null_mwir), not a
                       spatial mean. If None, CFG is disabled (guidance_scale
                       is ignored). Passing a flat spatial mean produces a
                       tile artefact — see DiTTrainer for the correct source.
        """
        B    = mwir.shape[0]
        x    = torch.randn(shape, device=device)
        dt   = 1.0 / num_steps

        if guidance_scale > 1.0 and null_cond is None:
            raise ValueError(
                "guidance_scale > 1.0 requires null_cond (the learned null MWIR "
                "embedding from DiTTrainer.null_mwir). "
                "Passing None disables CFG — set guidance_scale=1.0 or provide null_cond."
            )

        null = null_cond.expand(B, -1, -1, -1).to(device) if null_cond is not None else None
    ...
    def sample_heun(
        ...
        """
        Heun's method (2nd-order Runge-Kutta) ODE integration.

        Args:
            null_cond: learned null MWIR embedding (DiTTrainer.null_mwir).
                       Required when guidance_scale > 1.0. Must NOT be a
                       spatial mean — see sample_euler docstring for details.
        """
        if guidance_scale > 1.0 and null_cond is None:
            raise ValueError(
                "guidance_scale > 1.0 requires null_cond (the learned null MWIR "
                "embedding). Set guidance_scale=1.0 or provide null_cond."
            )
    ...
    def sample(self, model, mwir, shape, num_steps=20, guidance_scale=1.0,
               ...
               null_cond=None):
        """Convenience alias. Pass null_cond=trainer.null_mwir for CFG."""
        if method == 'euler':
