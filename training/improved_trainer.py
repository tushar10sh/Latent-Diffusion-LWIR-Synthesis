...
class ImprovedTrainer:
    ...
    def __init__(self, config: dict):
        ...
        # ── Scene context injection: persistent hook (no per-step monkey-patching) ──
        # Registering a pre-forward hook on time_embed once at init is ~20× faster
        # than creating a new nn.Module and monkey-patching every step.
        # The hook adds scene_ctx (set via self._scene_ctx before each forward call)
        # to the time embedding output, injecting global scene context into AdaGN.
        self._scene_ctx: Optional[torch.Tensor] = None

        def _time_embed_hook(module, input, output):
            if self._scene_ctx is not None:
                return output + self._scene_ctx
            return output

        self._time_embed_hook_handle = self.model.time_embed.register_forward_hook(
            _time_embed_hook
        )
        print("[ImprovedTrainer] Scene context hook registered on time_embed")
        ...
        self.use_amp   = (config.get('precision', 'float32') == 'bfloat16') \
                         and (self.device.type == 'cuda')
        self.amp_dtype = torch.bfloat16 if self.use_amp else torch.float32
        self.scaler    = GradScaler('cuda', enabled=False)  # kept for compat; never used
        if self.use_amp:
            print("[ImprovedTrainer] Precision: bfloat16")
        else:
            print("[ImprovedTrainer] Precision: float32")
    ...
    def train_step(self, batch):
        ...
        # Guard against NaN/Inf loss (bad data or numerical instability)
        if not torch.isfinite(total_loss):
            print(f"[Warning] NaN/Inf loss at step {self.global_step} — skipping update.")
        ...
        # bfloat16 does NOT need GradScaler — remove it to prevent
        # the scaler from skipping all optimizer steps on bfloat16.
        if self.use_amp:
            total_loss.backward()
    ...
    def _forward_with_scene_ctx(
        ...
        """
        Forward pass with scene context injected into the timestep embedding.

        Uses a persistent forward hook registered at __init__ time rather than
        creating a new nn.Module and monkey-patching every step. The hook adds
        self._scene_ctx to the time_embed output. This is ~20× faster in Python
        overhead and allows torch.compile to trace the model graph correctly.
        """
        self._scene_ctx = scene_ctx
        out = self.model(xt, t, mwir)
        self._scene_ctx = None   # clear so hook is a no-op outside this call
        return out
    ...
    def _generate_fn(self, mwir: torch.Tensor) -> torch.Tensor:
        ...
        scene_ctx = self.scene_encoder(mwir)
        B        = mwir.shape[0]
        lwir_ch  = self.cfg.get('lwir_channels', 1)
        H, W     = mwir.shape[2], mwir.shape[3]

        # ddim_sample_bridge expects 'model' as its first positional parameter,
        # not 'model_fn'. Pass a callable that injects scene context.
        def model_with_ctx(xt, t, m):
            return self._forward_with_scene_ctx(xt, t, m, scene_ctx)

        result = self.bridge_scheduler.ddim_sample_bridge(
            model=model_with_ctx,
            mwir=mwir,
            x_prior=x_prior,
            shape=(B, lwir_ch, H, W),          # use lwir_channels, not mwir channels
